import './common.css'
import './Visa.css'

function VISAtypes(){
    return (
        <div className='page'>
        <div className="subpage">
        <h1> VISA Types </h1>
        <hr></hr>
        <h3> STUDENT VISAS </h3>
        <hr></hr>
        <ul>
            <li>S-1 For higher education (including South Asian University and Nalanda University), provisional student visa, for a short term course and for those coming under student exchange program</li>
            <li>S-2 For school education in India</li>
            <li>S-3 For studying Yoga, vedic culture, Indian system of music and dance and buddist studies </li>
            <li>S-4 Theological Studies and Missionary students</li>
            <li>S-5 For Research scholar, visiting research faculty, research scholar on bilateral exchange programme, members of botanical scientific, anthropological etc. expeditions</li>
            <li>S-6 For those coming for internship in India (including those coming under French VIE programme)</li>
        </ul>
        <h3> Dependent Visas under student visa category: </h3>
        <ul>
            <li>     S-1X For dependent of student of regular courses in universities imparting higher education </li>
                 <li>   S-3X For dependent of student studying Yoga,  vedic culture, Indian system of music and dance and   Buddist studies </li>
                 <li>   S-5X For dependent of student for Research scholar, visiting research faculty, research scholar on bilateral       exchange programme,  members of botanical scientific, anthropological etc. expeditions</li>

        </ul>
        <h3>NOTE: *For dependents of those coming under S-1, S-3 and S-5 Visas only. For other category visas, no dependent visa will be granted*</h3>
        <hr></hr>
        <h3 > CONFERENCE VISAS </h3>
        <hr></hr>
        <ul>
            <li>    C-1 For participants of conferences organized by Government authorities </li>
            <li>    C-2 For participants of conferences organized by Non-Government authorities</li>

        </ul>
        <hr></hr>
        <h3 > EMPLOYMENT VISAS </h3>
        <hr></hr>
        <ul>
            <li> E-1 All cases of employment (except intra company transferees and employment in NGO’s)</li>
    <li>E-2 Intra company transferees</li>
    <li>E-3 For employment in NGO's</li>
    <li>E-4 For those coming to exclude projects in power and steel sectors</li>
        </ul>

    <h3> Dependent Visas under employment visa category:</h3>

        <ul>

            <li>E-1X For dependents of employment visa holders</li>
            <li>E-2X For dependents of  Intra company transferees</li>
            <li>E-3X For dependents of employees employed in NGO's</li>
            <li>E-4X For dependents of those coming to exclude projects in power and steel sectors</li>
        </ul>
        <hr></hr>
        <h3 > BUSINESS VISAS </h3>
        <hr></hr>
        <ul>
            <li> B-1 For those coming to establish an industrial/business venture, those required to travel frequently on long-term business activities and for investors</li>
    <li>B-2 For all other business activities (other than those covered under B-1 and sports persons)</li>
    <li>B-3 For sports persons and coaches including those engaged in commercial sports events in india on contract with remuneration</li>
    </ul>

<h3>Dependent Visas under business visa category:</h3>

<ul>
    <li>B-1X  For dependents of those coming to establish an industrial/business venture, those required to travel frequently on long-term business activities and for investors</li>
    <li>B-2 X For dependents of business person coming for all other business activities (other than those covered under B-1 and sports persons)</li>
        </ul>

        <hr></hr>
        <h3 >  DIPLOMATIC VISAS </h3>
        <hr></hr>
        <ul>
                <li> D-1 Diplomat assigned in India ( on diplomatic passport only)</li>
                <li>D-2 Diplomat on a visit to India for official purpose (on diplomatic passport only)</li>
                <li>D-3 Diplomat assigned to non-UN international organization in India</li>
                <li>UD-1 UN Diplomat assigned to India</li>
                <li>UD-2 UN Diplomat on a visit to India</li>
        </ul>

<h3>Dependent Visas under diplomatic visa category:</h3>

            <ul>
                    <li>D-1X Dependent of diplomat (Diplomat assigned in India (on diplomatic passport only))</li>
                    <li>D-2X Dependent of diplomat (Diplomat on a visit to India for official purpose (on diplomatic passport only))</li>
                    <li>D-3X Dependent of diplomat (Diplomat assigned to non-UN international organization in India)</li>
                    <li>UD-1X Dependent of UN diplomat assigned to India</li>
                    <li>UD-2X Dependent of UN diplomat on a visit to India</li>
            </ul>

        <hr></hr>
        <h3 >  DOUBLE ENTRY VISAS (ONLY FOR BAGLADESHI NATIONALS)</h3>
        <hr></hr>
        <ul>
            <li>X-Double Entry - (Double Entry Visa)</li>
        </ul>
        <hr></hr>
        <h3 >  DIPLOMATIC VISAS </h3>
        <hr></hr>
        <ul><li>eXM- e-Emergency X-Misc VISA</li>
        </ul>
        <hr></hr>
        <h3 >  ENTRY VISAS </h3>
        <hr></hr>
        <ul><li>    X-1 - For persons of Indian origin and spouse and children of an Indian citizen / Person of Indian origin / OCI card (other than those registered as OCI cardholder)</li>
        <li>X-2 - For those coming to join Auroville Foundation, Aurobindo Ashram, Missionaries of Charity,Templates or similar approved organizations</li>
        <li>X-Misc - For those categories for which X-Misc visa has been specified in various chapters of the Visa manual and for purposes which are not covered by any other visa category</li>
        </ul>
        <hr></hr>
        <h3 >  e-VISAS </h3>
        <hr></hr>
        <ul>
            <li>e-BV - (e-Business Visa)</li>
            <li>e-CV - (e-Conference Visa)</li>
            <li>e-Med V - (e-Medical Visa)</li>
            <li>e-Med X V - (e-Medical Attendant Visa)</li>
            <li>e-TV - (e-Tourist Visa)</li>

        </ul>
        <hr></hr>
        <h3 >  FILMS VISAS </h3>
        <hr></hr>
        <ul><li>F - For foreign nationals coming for shooting of feature films, reality TV and web shows/series, commercial TV serials/shows and web shows / series and for location recce</li>
        </ul>
        <hr></hr>
        <h3 >  JOURNALIST VISAS </h3>
        <hr></hr>
        <ul><li>
        J-1 - For Journalists accredited in India and also journalists visiting India for any purpose including tourism, conference etc</li>
    <li>J-1X - Dependent of journalists accredited in India and also for journalists visiting India for any purpose including tourism, conference etc.</li>
    <li>J-2- Foreign nationals involved in shooting advertisement / documentary films etc.</li>

        
        </ul>
        <hr></hr>
        <h3 >  LONG TERM VISAS </h3>
        <hr></hr>
        <ul><li>    L- Long Term Visa
        </li>
        </ul>
        <hr></hr>
        <h3 >  MEDICAL VISAS </h3>
        <hr></hr>
        <ul><li>    MED-1- For patients</li>
     <li>MED-2- For foreign nationals coming as Medical Attendants</li>

        </ul>
        <hr></hr>
        <h3 >  MISC SP VISAS </h3>
        <hr></hr>
        <ul><li> MISC - Misc SP Permission</li>
        </ul>
        <hr></hr>
        <h3 >  MISSIONARY VISAS </h3>
        <hr></hr>
        <ul><li>    M-1- Missionary / religious worker of approved organization</li>
     <li>M-1X- Dependent of missionary / religious worker of approved organization</li>

        </ul>
        <hr></hr>
        <h3 >  MOUNTAINEERING VISAS </h3>
        <hr></hr>
        <ul><li>MX – Mountaineering expeditions</li>
        </ul>
        <hr></hr>
        <h3 >  OFFICIAL VISAS </h3>
        <hr></hr>
        <ul><li>        O-1- Non-diplomatic official assigned to a diplomatic mission in India (official/service/special passport only )</li>
        <li>O-1X- Dependent of non-diplomatic official assigned in India</li>
        <li>O-2- Non-diplomatic official on a visit to India for official purpose</li>
        <li>O-2X- Dependent of non-diplomatic official visiting India</li>
        <li>UO-1- UN non-diplomatic official assigned to India</li>
        <li>UO-1X- Dependent of UN non-diplomatic official assigned in India</li>
        <li>UO-2 - UN non-diplomatic official on a visit to India</li>
        <li>UO-2X- Dependent of UN non-diplomatic official on a visit to India</li>

        </ul>
        <hr></hr>
        <h3 >  PILGRIM VISAS (ONLY FOR PAKISTAN NATIONALS) </h3>
        <hr></hr>
        <ul><li>        PG- 1- Pilgrim Visa (under religious protocol)</li>
        <li>PG-2- Group pilgrim visa for minority communities in Pakistan</li>

        </ul>
        <hr></hr>
        <h3 >  TOURIST VISAS </h3>
        <hr></hr>
        <ul><li>T - For recreation, sightseeing, casual visit to meet friends or relatives, attending a short term yoga programme and short duration medical treatment including medical under Indian systems of medicine</li>
        </ul>
        <hr></hr>
        <h3 >  TRANSIT VISAS </h3>
        <hr></hr>
        <ul><li>TR- Travelling through India to a destination outside India</li>
        </ul>
        <hr></hr>
        <h3 >  VISA-ON-ARRIVAL </h3>
        <hr></hr>
        <ul><li>VoA- Visa on arrival for Japanaese and South Korean nationals</li></ul>
        <hr></hr>
        <h3 >  VISITOR VISAS (ONLY FOR PAKISTAN NATIONALS) </h3>
        <hr></hr>
        <ul><li>V - Visitor Visa</li></ul>
        <hr></hr>
        
        </div>
        </div>
    )
}
export default  VISAtypes